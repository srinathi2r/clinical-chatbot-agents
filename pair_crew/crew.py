...
def build_crew(query: str, verbose: bool = False) -> tuple[Crew, list[Task]]:
    ...
    task_specialist = Task(
        description=(
            ...
            "11. For any NBM CBG classification, call the NBM CBG Classifier and "
            "cite the retrieved SGH NBM Guidance document and page for the "
    ...
    task_safety = Task(
        description=(
            ...
            "Safety Checker reports cbg_nbm_citation, force one rework cycle using "
            "the retrieved NBM source passage. If the NBM citation is still absent "
            "after rework, refuse with this exact text: 'Citation lost from source "
            "guideline, please consult senior clinician.' For CBG 4.0, do not "
            "release an answer that only negates hypoglycaemia. Preserve the in "
            "target range or lower boundary of target label.\n\n"
